...
const Authorization = ({ getDataFromSocial, getTypeOfAuth }) => {
  ...
  const handleOnChange = e => {
    ...
    switch (name) {
      case 'email':
        setEmail(value);
        schemaEmail.isValid(value).then(function (valid) {
          if (valid) {
            setEmailError('');
          } else {
      ...
      case 'password':
        setPassword(value);
        schemaPassword.isValid(value).then(function (valid) {
          if (valid) {
            setPasswordError('');
          } else {
            setPasswordError(
              'Пароль должен содержать буквы в верхнем и нижнем регистре и спец. симолы',
            );
          }
        });
  ...
  const handleAuthFromSocial = (
    authEmailFromSocial,
    authPasswordFromSocial,
    authImgFromSocial,
    authNameFromSocial,
  ) => {
  ...
  right: "0"}}>{passwordError}</div>} */}
              {isPasswordVisible ? (
                <VisibilityIcon className={s.password} onClick={togglePasswordVisability} />
              ) : (
          ...
          {isLoginType ? (
            <div className={s.authLoginBtnsWrapper}>
              <button className={s.authBtn + ' ' + s.authBtnActive} type="submit">
                Войти
              </button>
              <button className={s.authBtn} type="button" onClick={toggleIsLoginType}>
                Регистрация
              </button>
            </div>
          ) : (
            <div className={s.authAuthBtnsWrapper}>
              <button className={s.authBtn + ' ' + s.authBtnActive} type="submit">
                Регистрация
              </button>
              <button className={s.authBtn} type="button" onClick={toggleIsLoginType}>
